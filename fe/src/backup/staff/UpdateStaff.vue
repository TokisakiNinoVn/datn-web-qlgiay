<template>
  <div v-if="show" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white p-4 rounded-lg shadow-lg w-1/4">
      <h2 class="text-xl font-bold text-black p-2 bg-yellow-400 mb-4">Cập nhật thông tin Nhân viên</h2>
      <form @submit.prevent="handleUpdate">
        <input v-model="formData.username" type="text" placeholder="Tên đăng nhập" class="border p-2 mb-2 w-full" disabled />
        <input v-model="formData.email" type="email" placeholder="Email" class="border p-2 mb-2 w-full" />
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleUpdate = () => {
      const updatedData = {
        ...formData.value,
        role: roles.value.find((role) => role.id === formData.value.roleId) || null, // Gắn lại role dựa trên roleId
      };
      emit('updateStaff', updatedData);
    };
    ...
    return { formData, roles, handleUpdate, close };
  },
};
</script>
